...
- This is the most DRY approach I could find and avoids creating several additional type variables and yet having to type several variable names which are usually just a reformat of the "type" string;
...
By writing like this I avoid having to write a `var` and an `interface` for the same thing. It's all in one place, less duplication! One downside is the inability of writing root optional keys: it has to be `undefined`. (PS avoid `null` since one never knows when will write `typeof x === 'object`).
...
- Either you pass all of the "type hints", either none. There's no middle ground;
- If you pass no type hints, then you'll have to annotate some key variables and pray the inference gets it. Always aim for this case. (this is should be a typescript mantra: infer everything!)
...
Nothing special here. I'm 100% not a fan of baking dozens of functions into a switch-case, but we will come to that later.
...
- The whole redux thing is mostly an abstraction for calling functions:
  ...
  - This allows a clearer implementation of a middleware pattern, which allows... _things_ (like the redux dev tools); Oh, subscriptions too.
...
This approach has been exported into [yet another javascript library](https://github.com/wkrueger/redutser), just in case you are curious. I have also baked in a couple of another useful stuff in. I'd also recommend [rematch](https://github.com/rematch/rematch), which seems quite similar and straightforward.
...
Rematch suggests the grouping of async dispatcheables into a dict they call `effects`. Let's do something in the same lines
...
```typescript
export function createEffects<MB extends Meatball>(
  meatball: MB
) {
  return function<
    FX extends {
